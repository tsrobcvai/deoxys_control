...
bool OSCPositionImpedanceController::ParseMessage(
  ...
  if (first_time_print || new_disable != disable_inertial_decoupling_) {
    if (new_disable) {
      std::cout << "[Deoxys] OSC Position Controller: Inertial decoupling is DISABLED." << std::endl;
    } else {
      std::cout << "[Deoxys] OSC Position Controller: Inertial decoupling is ENABLED." << std::endl;
    }
